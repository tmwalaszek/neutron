...
class SecurityGroupDbMixin(ext_sg.SecurityGroupPluginBase,
    ...
    def update_security_group_rule(self, context, id, security_group_rule):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            original_rule = self._make_security_group_rule_dict(
                sg_rule.db_obj)
            ...
            # Check that remote_group_id, if changed, still exists for
            # the tenant, same as on create.
            if ('remote_group_id' in requested_fields and
                    merged_rule['remote_group_id']):
                self._check_security_group(
                    context, merged_rule['remote_group_id'],
            ...
            updated_rule = self._make_security_group_rule_dict(
                sg_rule.db_obj)
            self._registry_publish(
                resources.SECURITY_GROUP_RULE,
                events.PRECOMMIT_UPDATE,
