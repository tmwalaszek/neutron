...
# The ACL relevant fields, which an SG rule dict always carries.
SGR_ACL_FIELDS = {'direction': 'ingress', 'ethertype': 'IPv4',
...
class TestOVNMechanismDriver(TestOVNMechanismDriverBase):
    ...
    def test__process_sg_rule_notifications_sgr_update(self, mock_bump):
        original_rule = dict(SGR_ACL_FIELDS, id='sgr_id',
                             security_group_id='sg_id',
                             remote_ip_prefix='10.0.0.0/24',
                             normalized_cidr='10.0.0.0/24')
        updated_rule = dict(SGR_ACL_FIELDS, id='sgr_id',
                            security_group_id='sg_id',
    ...
    @mock.patch.object(ovn_revision_numbers_db, 'bump_revision')
    def test__process_sg_rule_notifications_sgr_update_acl_unchanged(
            self, mock_bump):
        original_rule = dict(SGR_ACL_FIELDS, id='sgr_id',
                             security_group_id='sg_id',
                             remote_ip_prefix='10.0.0.0/24',
                             normalized_cidr='10.0.0.0/24',
                             description='before')
        updated_rule = dict(original_rule, description='after')
        with mock.patch.object(
                self.mech_driver,
                '_sg_has_rules_with_same_normalized_cidr') as has_same_rules, \
                mock.patch.object(
            ...
            self.mech_driver._process_sg_rule_notification(
                ...
                payload=events.DBEventPayload(
                    self.context, states=(original_rule, updated_rule)))
            ovn_acl_up.assert_not_called()
            has_same_rules.assert_not_called()
            self.mech_driver._ovn_client._nb_idl.transaction.\
                assert_not_called()
            mock_bump.assert_called_once_with(
                mock.ANY, updated_rule, ovn_const.TYPE_SECURITY_GROUP_RULES)
